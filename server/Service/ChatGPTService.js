...
module.exports = class ChatGPTService {
  ...
  static async summaryWithChatGpt(input) {
    ...
    const prompt = `Create compelling marketing description to promote your apartment and attract potential buyers or tenants. Write captivating marketing summary in 2 lines for: ${apartment}. Return the summary in text format`;
    const response = await openai.createCompletion({
      model: "text-davinci-003",
      prompt: prompt,
      max_tokens: 1000,
