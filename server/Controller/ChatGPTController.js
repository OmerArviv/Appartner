...
router.route("/conver").post(async (request, response) => {
  const { data } = request.body;
  if (!data) {
    return response.status(403).send({});
  }

  const dataJson = await ChatGPTService.convWithChat(data);
...
router.route("/shortcut").post(async (request, response) => {
  ...
  const dataJson = await ChatGPTService.shortcutWithChatGpt(data);
...
router.route("/summary").post(async (request, response) => {
  const { data } = request.body;
  if (!data) {
    return response.status(403).send({});
  }

  const dataJson = await ChatGPTService.summaryWithChatGpt(data);
